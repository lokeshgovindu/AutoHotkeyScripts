...
;
; * v2.0.4
; - Added option to change the selection of ListBox items using
;   Up/Down keys from Edit control itself, no need to hit enter
...
dirExplorer = C:\Windows\explorer.exe
...
IfNotEqual 1, -scex
{
	...
	Subclass.SetFunction(FilenameHwnd, WM_KEYDOWN, "Filename_WM_KEYDOWN")
}
...
; Commented the below lines of code
; We don't need to set the focus to ListBox

;~ GuiControlGet, focusControl, 1:Focus
;~ If focusControl = Edit1
;~ {
	;~ FileAppend, [ButtonOPEN] SetFocus to OpenTarget ListBox`n, *
	;~ GuiControl, Focus, OpenTarget
	;~ GuiControl, 1:Disable, OpenTarget
	;~ GuiControl, 1:Disable, ButtonOPENDIR
	;~ GuiControl, 1:Disable, ButtonSCANSTARTMENU
...
OnKeyDown(wParam, lParam, msg, hwnd) {
	...
	if (hwnd = FilenameHwnd) {
		;~ FileAppend, [OnKeyDown] hwnd = FilenameHwnd`n, *
